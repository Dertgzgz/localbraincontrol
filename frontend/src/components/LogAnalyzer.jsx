...
const LogAnalyzer = ({ t, lang }) => {
  ...
  useEffect(() => {
    // Solo ejecutar si source está definido y es válido
    ...
    const fetchPeek = async () => {
      ...
      try {
        ...
        setPeekContent(data.content || 'No content available');
      } catch (e) {
        console.error('Error fetching peek logs:', e);
        setPeekContent('Failed to fetch peek logs.');
      } finally {
        setIsLoadingPeek(false);
      }
    };
    fetchPeek();
  }, [source]); // Dependencia en source

  const handleAnalyze = async () => {
    if (!source) return; // No ejecutar si source no está definido
    ...
    <div className="space-y-8 animate-in fade-in duration-700">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-4 gap-8">
        ...
        <div className="lg:col-span-1 space-y-6">
          ...
          {/* Botón de prueba para generar error */}
          <button 
            onClick={() => {
              try {
                ...
                // El error será capturado por el event listener global
                console.error('Error de prueba generado:', e);
              }
            }}
            className="w-full py-3 rounded-2xl bg-red-500/10 text-red-400 font-bold text-xs border border-red-500/20 hover:bg-red-500/20 transition-all flex items-center justify-center gap-2"
          >
          ...
          <AnimatePresence mode="wait">
            {isAnalyzing ? (
              ...
              >
                ...
                <p className="text-gray-500 text-sm font-medium tracking-wide">Processing {source.toUpperCase()} events via {modelProvider.toUpperCase()} neural path...</p>
              </motion.div>
            ) : result ? (
              <motion.div 
                key="result"
                ...
                <div className="bg-gradient-to-br from-[#111] to-[#0a0a0a] border border-gray-800 rounded-[2.5rem] p-10 shadow-3xl relative overflow-hidden group">
                  ...
                  <div className="flex items-center gap-4 mb-8">
                    ...
                    <div>
                      ...
                      <p className="text-[10px] font-bold text-gray-500 uppercase tracking-widest mt-1">Diagnostic Artifact Generated by {modelProvider.toUpperCase()}</p>
                    </div>
                  </div>
